...
class ESM2Classifier(nn.Module):
    ...
    def forward(self, batch_tokens, attention_mask, collect_attention=False):
        ...
        with torch.no_grad():
            ...
            token_representations = results["representations"][6]  # Shape: [batch_size, seq_len, embedding_dim]

        # Transpose for MultiheadAttention compatibility
        x = token_representations.transpose(0, 1)  # Shape: [seq_len, batch_size, embedding_dim]
        ...
        x_mean = x.mean(dim=0)  # Shape: [batch_size, embedding_dim]
